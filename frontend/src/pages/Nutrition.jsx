import React, { useState } from "react";
import Layout from "../components/Layout";
import { PieChart, Pie, Cell, Legend, ResponsiveContainer } from "recharts";
import { Pencil, Trash2, Plus, Minus } from "lucide-react";

const Nutrition = () => {
  const [editIndex, setEditIndex] = useState(null);
  const [editValue, setEditValue] = useState("");

  const foodList = [
    { name: "Egg", calories: 78 },
    { name: "Bread Slice", calories: 66 },
    { name: "Apple", calories: 95 },
    { name: "Rice (1 cup)", calories: 200 },
    { name: "Grilled Chicken", calories: 165 },
  ];

  const [meals, setMeals] = useState({
    Breakfast: [],
    Lunch: [],
    Dinner: [],
  });

  // ✅ Add new food item
  const handleAddFood = (meal, food) => {
    setMeals((prev) => ({
      ...prev,
      [meal]: [...prev[meal], { ...food, quantity: 1 }],
    }));
  };

  // ✅ Increase / decrease quantity
  const handleQuantityChange = (meal, index, type) => {
    setMeals((prev) => {
      const updated = { ...prev };
      updated[meal] = prev[meal].map((item, i) =>
        i === index
          ? {
              ...item,
              quantity:
                type === "inc"
                  ? item.quantity + 1
                  : item.quantity > 1
                  ? item.quantity - 1
                  : 1,
            }
          : item
      );
      return updated;
    });
  };

  // ✅ Delete item
  const handleDelete = (meal, index) => {
    setMeals((prev) => {
      const updated = { ...prev };
      updated[meal] = prev[meal].filter((_, i) => i !== index);
      return updated;
    });
  };

  // ✅ Edit item
  const handleEdit = (meal, index) => {
    setEditIndex({ meal, index });
    setEditValue(meals[meal][index].name);
  };

  // ✅ Save edited item
  const handleSaveEdit = (meal, index) => {
    setMeals((prev) => {
      const updated = { ...prev };
      const food = foodList.find((f) => f.name === editValue);
      if (food) updated[meal][index] = { ...food, quantity: 1 };
      return updated;
    });
    setEditIndex(null);
  };

  const chartData = [
    { name: "Protein", value: 30 },
    { name: "Carbs", value: 50 },
    { name: "Fats", value: 20 },
  ];

  const COLORS = ["#22c55e", "#3b82f6", "#facc15"];

  const totalCalories = Object.values(meals)
    .flat()
    .reduce((sum, item) => sum + item.calories * item.quantity, 0);

  return (
    <Layout>
      <div className="flex flex-col lg:flex-row bg-white h-[calc(100vh-64px)] overflow-hidden">
        {/* Left Side - Scrollable Form and Cards */}
        <div className="flex-1 p-6 border-r border-gray-200 overflow-y-auto scrollbar-hide">
          {/* Add Calories Form */}
          <h2 className="text-xl font-semibold mb-4">Add Calories</h2>

          <div className="flex flex-col lg:flex-row gap-4 mb-6">
            {["Breakfast", "Lunch", "Dinner"].map((meal) => (
              <div key={meal} className="flex-1">
                <label className="block text-sm font-medium mb-1">{meal}</label>
                <select
                  className="w-full border rounded-lg px-3 py-2 cursor-pointer"
                  onChange={(e) => {
                    const food = foodList.find(
                      (f) => f.name === e.target.value
                    );
                    if (food) handleAddFood(meal, food);
                  }}
                >
                  <option value="">Select Food</option>
                  {foodList.map((food, i) => (
                    <option key={i} value={food.name}>
                      {food.name} - {food.calories} kcal
                    </option>
                  ))}
                </select>
              </div>
            ))}
          </div>

          {/* All Details Section */}
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">All Details</h3>
            <span className="bg-blue-100 text-blue-600 text-sm px-3 py-1 rounded-full">
              Total: {totalCalories} Kcal
            </span>
          </div>

          {/* Meal Cards */}
          <div className="space-y-4 pb-8">
            {Object.keys(meals).map((meal) => (
              <div
                key={meal}
                className="border border-gray-200 rounded-lg p-4 shadow-sm"
              >
                <h4 className="font-semibold mb-3">{meal}</h4>
                {meals[meal].length === 0 ? (
                  <p className="text-gray-400 text-sm">No items added</p>
                ) : (
                  <ul className="space-y-2">
                    {meals[meal].map((item, index) => (
                      <li
                        key={index}
                        className="flex flex-wrap items-center justify-between bg-gray-50 p-2 rounded-lg"
                      >
                        <div>
                          {editIndex?.meal === meal &&
                          editIndex?.index === index ? (
                            <select
                              className="border rounded-md px-2 py-1 text-sm cursor-pointer"
                              value={editValue}
                              onChange={(e) => setEditValue(e.target.value)}
                            >
                              {foodList.map((food) => (
                                <option key={food.name} value={food.name}>
                                  {food.name}
                                </option>
                              ))}
                            </select>
                          ) : (
                            <p className="font-medium">
                              {item.name} ({item.quantity}x)
                            </p>
                          )}
                          <p className="text-sm text-gray-500">
                            {item.calories * item.quantity} kcal
                          </p>
                        </div>

                        <div className="flex items-center gap-2 mt-2 lg:mt-0">
                          <button
                            onClick={() =>
                              handleQuantityChange(meal, index, "dec")
                            }
                            className="p-1 rounded-md border hover:bg-gray-100 cursor-pointer"
                          >
                            <Minus size={14} />
                          </button>

                          <span className="px-2 text-sm font-semibold">
                            {item.quantity}
                          </span>

                          <button
                            onClick={() =>
                              handleQuantityChange(meal, index, "inc")
                            }
                            className="p-1 rounded-md border hover:bg-gray-100 cursor-pointer"
                          >
                            <Plus size={14} />
                          </button>

                          {editIndex?.meal === meal &&
                          editIndex?.index === index ? (
                            <button
                              onClick={() => handleSaveEdit(meal, index)}
                              className="text-green-500 hover:text-green-600 font-semibold text-sm cursor-pointer"
                            >
                              Save
                            </button>
                          ) : (
                            <button
                              onClick={() => handleEdit(meal, index)}
                              className="text-blue-500 hover:text-blue-600 cursor-pointer"
                            >
                              <Pencil size={16} />
                            </button>
                          )}

                          <button
                            onClick={() => handleDelete(meal, index)}
                            className="text-red-500 hover:text-red-600 cursor-pointer"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </li>
                    ))}
                  </ul>
                )}
              </div>
            ))}
          </div>
        </div>

        {/* Right Side - Fixed Section (No Scroll) */}
        <div className="w-full lg:w-1/4 p-6 bg-gray-50 flex flex-col gap-6 border-l border-gray-200">
          <div>
            <h4 className="text-lg font-semibold mb-1">BMI</h4>
            <p className="text-2xl font-bold text-blue-600 text-center">22.8</p>
          </div>

          <div>
            <h4 className="text-lg font-semibold mb-1">BMR</h4>
            <p className="text-2xl font-bold text-green-600 text-center">
              1550 Kcal
            </p>
          </div>

          <div>
            <h4 className="text-lg font-semibold mb-3 text-center">
              Food Intake
            </h4>
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie
                  data={chartData}
                  cx="50%"
                  cy="50%"
                  innerRadius={45}
                  outerRadius={60}
                  paddingAngle={6}
                  dataKey="value"
                >
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS[index]} />
                  ))}
                </Pie>
                <Legend verticalAlign="bottom" height={36} />
              </PieChart>
            </ResponsiveContainer>
          </div>

          <button className="bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-lg shadow transition-all self-center w-full mt-1 cursor-pointer">
            Calculate All
          </button>
        </div>
      </div>
    </Layout>
  );
};

export default Nutrition;
