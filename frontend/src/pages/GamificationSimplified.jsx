...
const GamificationSimplified = () => {
  ...
  const loadGamificationData = async () => {
    ...
    try {
      // Load profile, challenges, leaderboard, and rank in parallel
      const [profileRes, challengesRes, leaderboardRes, rankRes] = await Promise.all([
      ...
      // Set leaderboard (limited to top 3)
  ...
  const handleStartChallenge = async (challengeId) => {
    ...
    try {
      console.log('📡 Sending challenge start request...');
      ...
      if (res?.data?.ok) {
        ...
        console.log('⚠️ Backend error:', res?.data);
        showToast(res?.data?.message || 'Failed to start challenge');
      }
    } catch (err) {
      console.log('❌ Start challenge failed:', err?.response?.data || err.message);
  ...
  const handleDeleteChallenge = async (challengeId) => {
    ...
    if (!token) {
      ...
      showToast('Challenge deleted locally');
      return;
    }
    ...
    try {
      const res = await axios.post(`${backendUrl}/api/gamification/challenge/delete`, { challengeId }, { headers });
      if (res?.data?.ok) {
        await loadGamificationData();
        showToast('Challenge deleted successfully');
      } else {
        showToast(res?.data?.message || 'Failed to delete challenge');
      }
    } catch (err) {
      console.error('Delete challenge error:', err);
      // Fallback to local deletion if backend fails
      setMyChallenges(myChallenges.filter((c) => c.id !== challengeId));
      showToast('Challenge deleted locally (backend unavailable)');
    }
  };
  ...
  return (
    <Layout>
      <div className="flex flex-col lg:flex-row bg-white h-[calc(100vh-64px)] overflow-hidden">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-lg font-bold text-blue-500">
                        {challenge.xp_points || challenge.xp} XP
                      </span>
                    </div>
                    <button
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm font-bold text-blue-500">
                                {challenge.xp || challenge.xp_points} XP
                              </span>
                            </div>
                          </div>
                          <span
        ...
        {/* Right Section - Leaderboard (Top 10) */}
        <div className="w-full lg:w-80 bg-gray-50 border-l border-gray-200 p-3 sm:p-6 overflow-y-auto">
          <div className="mb-6">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Top 10 Leaderboard</h3>
...
export default GamificationSimplified;